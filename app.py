...
def recibir_datos():
    # Permitir tanto JSON formal como texto plano parseado a JSON por seguridad con ESP32
    ...
    if not datos:
        # Intento de emergencia si llega como form-data o texto plano
        if request.form:
            datos = request.form.to_dict()
        else:
            return jsonify({"status": "error", "mensaje": "No se recibieron datos válidos"}), 400
    
    # Extraemos los campos enviados por el dispositivo
    temperatura = datos.get('temperatura', 0.0)
    ritmo_cardiaco = datos.get('ritmo_cardiaco', 0)
    accel_x = datos.get('accel_x', 0.0)
    accel_y = datos.get('accel_y', 0.0)
    accel_z = datos.get('accel_z', 0.0)
    efecto_hall = datos.get('efecto_hall', 0)
    latitud = datos.get('latitud', 0.0)
    longitud = datos.get('longitud', 0.0)
    velocidad_kmh = datos.get('velocidad_kmh', 0.0)
    fecha_registro = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Guardado seguro en la base de datos SQLite
    try:
        conn = obtener_conexion_db()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO telemetria (
                temperatura, ritmo_cardiaco, accel_x, accel_y, accel_z, 
        ...
        print(f"✅ [DB Guardada] Temp: {temperatura}°C | HR: {ritmo_cardiaco} bpm | Lat/Lon: {latitud}, {longitud}")
        return jsonify({"status": "success", "mensaje": "Datos almacenados correctamente"}), 200
